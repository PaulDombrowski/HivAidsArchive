...
function UploadForm() {
  ...
  const handleCategoryChange = (cat) => {
    if (category.includes(cat)) {
      setCategory(category.filter(c => c !== cat)); // Deselect
    } else {
      setCategory([...category, cat]); // Select
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!title || !description || !category.length || !type || !uploader || !mood) { 
  ...
  return (
    <div className="upload-container">
      <form onSubmit={handleSubmit} className={`upload-form ${success ? "success" : ""}`}>
        <label>
  ...
  <div className="category-checkboxes">
    {[
      ...
      <div
        key={cat}
        className={`category-item ${category.includes(cat) ? "active" : ""}`}  // Apply 'active' class if selected
        onClick={() => handleCategoryChange(cat)}  // Toggle selection on click
      >
        {cat}
      </div>
    ))}
    ...
    <option value="Other">Other</option>
        ...
        {success && <p className="success-message">Thanks for your contribution!</p>}
